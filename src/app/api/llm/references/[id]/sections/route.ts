...
export async function GET(request: Request, props: { params: Promise<{ id: string }> }) {
  try {
    ...
    const payload = {
      ...
      count: reference.sections.length,
      sections: reference.sections.map((section) => ({
        sectionId: section.sectionId,
        heading: section.heading,
        mode: section.mode,
        tokenEstimate: section.tokenEstimate,
        href: `/api/llm/references/${reference.id}/sections/${section.sectionId}`,
      })),
      links: {
        self: `/api/llm/references/${reference.id}/sections`,
    ...
    console.error('Failed to load reference sections', error);
    return jsonError('Failed to load reference sections');
  }
}
